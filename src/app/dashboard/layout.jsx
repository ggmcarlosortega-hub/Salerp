...
const UserIcon = ({ letra }) => (
  <div className="w-9 h-9 rounded-full bg-gray-600 flex items-center justify-center font-bold text-white text-sm shadow-md transition-transform duration-200">
    {letra || "N"}
  </div>
);
...
export default function DashboardLayout({ children }) {
  ...
  useEffect(() => {
    if (usuarioId) {
      fetch(`http://localhost:3001/api/actividad/mas-usado/${usuarioId}`, { headers: getHeaders() })
        .then(res => res.json())
        .then(data => {
    ...
    <div className="h-screen w-full flex flex-col font-sans overflow-hidden">
      ...
      <div className="w-full flex flex-grow overflow-hidden">
        ...
        <aside className={`w-1/4 max-w-[200px] min-w-[120px] ${appBlue} text-white py-8 px-4 flex flex-col items-center gap-10 shadow-inner z-0 overflow-y-auto`}>
          {esAdmin ? (
            <>
              <SidebarItem href="/dashboard/clientes" icon={ClienteIcon} label="Clientes" title="Agregar clientes" isHighlighted={moduloMasUsado === "clientes"} onClick={() => registrarAcceso("clientes")} />
              <SidebarItem href="/dashboard/maquinaria" icon={MaquinariaIcon} label="Maquinaria" title="Estados de la maquinaria" isHighlighted={moduloMasUsado === "maquinaria"} onClick={() => registrarAcceso("maquinaria")} />
              <SidebarItem href="/dashboard/productos" icon={ProductosIcon} label="Productos" title="Detalles de los productos" isHighlighted={moduloMasUsado === "productos"} onClick={() => registrarAcceso("productos")} />
              <SidebarItem href="/dashboard/gastos" icon={GastoIcon} label="Gastos" title="Análisis de los gastos" isHighlighted={moduloMasUsado === "gastos"} onClick={() => registrarAcceso("gastos")} />
              <SidebarItem href="/dashboard/proveedor" icon={ProveedorIcon} label="Proveedor" title="Gestion de Proveedor" isHighlighted={moduloMasUsado === "proveedor"} onClick={() => registrarAcceso("proveedor")} />
              <SidebarItem href="/dashboard/fabricacion" icon={FabricacionIcon} label="Fabricacion" title="Ordenes de fabricacion" isHighlighted={moduloMasUsado === "fabricacion"} onClick={() => registrarAcceso("fabricacion")} />
              <SidebarItem href="/dashboard/compras" icon={ComprasIcon} label="Compras" title="Compras a proveedores" isHighlighted={moduloMasUsado === "compras"} onClick={() => registrarAcceso("compras")} />
            </>
          ) : (
            <>
